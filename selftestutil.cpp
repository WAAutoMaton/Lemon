...
void SelfTestUtil::makeSelfTest(QWidget *widget, Contest *contest)
{
    ...
    for (int i = 0; i < taskList.size(); i++) {
...
#ifdef LEMON_OS_WIN32
        QFile file(Settings::selfTestPath() + taskList[i]->getProblemTile() + QDir::separator()
                   + "check.bat");
        if (!file.open(QFile::WriteOnly | QFile::Text)) {
            QApplication::restoreOverrideCursor();
            QMessageBox::warning(widget, tr("Lemon"), tr("Cannot write check.bat"),
                                 QMessageBox::Ok);
            return;
        }
        QFile dummy(Settings::selfTestPath() + taskList[i]->getProblemTile() + QDir::separator()
                    + "enter");
...
#ifdef LEMON_OS_WIN32
            QFile::copy(QCoreApplication::applicationDirPath() + "/judge/realjudge_win32.exe",
                        Settings::selfTestPath() + taskList[i]->getProblemTile() + QDir::separator()
                            + "realjudge.exe");
            QProcess::execute(QString("attrib -R \"") + Settings::selfTestPath()
                              + taskList[i]->getProblemTile() + QDir::separator() + "realjudge.exe"
                              + "\"");
#endif
#ifdef LEMON_OS_UNIX
            QFile::copy(QCoreApplication::applicationDirPath() + "/judge/realjudge_linux",
                        Settings::selfTestPath() + taskList[i]->getProblemTile() + QDir::separator()
                            + "realjudge");
            QProcess::execute(QString("chmod +wx \"") + Settings::selfTestPath()
                              + taskList[i]->getProblemTile() + QDir::separator() + "realjudge"
                              + "\"");
#endif
        }
        ...
        for (int j = 0; j < testCaseList.size(); j++) {
            ...
            for (int k = 0; k < inputFiles.size(); k++) {
                ...
                if (!taskList[i]->getStandardInputCheck()
                    && taskList[i]->getTaskType() == Task::Traditional) {
                    out << QString("copy \"%1\" \"%2\" >nul")
                               .arg(inputFile, taskList[i]->getInputFileName())
                        << endl;
                }
                ...
                if (taskList[i]->getComparisonMode() == Task::LineByLineMode) {
                    out << QString("fc \"%1\" \"%2\"").arg(outputFileName, outputFile) << endl;
                }
                if (taskList[i]->getComparisonMode() == Task::IgnoreSpacesMode) {
                    out << QString("fc \"%1\" \"%2\" /W").arg(outputFileName, outputFile) << endl;
                }
                ...
                if (taskList[i]->getComparisonMode() == Task::RealNumberMode) {
                    out << QString("realjudge.exe \"%1\" \"%2\" \"%3\"")
                               .arg(outputFileName)
                               .arg(outputFile)
                               .arg(taskList[i]->getRealPrecision())
                        << endl;
                }
                if (taskList[i]->getComparisonMode() == Task::SpecialJudgeMode) {
                    out << QString("\"%1\" \"%2\" \"%3\" \"%4\" \"%5\" \"%6\" \"%7\"")
                               .arg(QFileInfo(taskList[i]->getSpecialJudge()).fileName(), inputFile,
                                    outputFileName, outputFile,
                                    QString("%1").arg(testCaseList[j]->getFullScore()), "_score",
                                    "_message")
                        << endl;
                ...
                if (!taskList[i]->getStandardInputCheck()
                    && taskList[i]->getTaskType() == Task::Traditional) {
                    out << QString("cp %1 %2").arg(inputFile, taskList[i]->getInputFileName())
                        << endl;
                }
                out << QString("echo \"Test Case: %1\"").arg(index) << endl;
                if (taskList[i]->getTaskType() == Task::Traditional) {
                ...
                if (taskList[i]->getComparisonMode() == Task::IgnoreSpacesMode) {
                    QString arg = QString(" \"%1\" \"%2\"").arg(outputFileName, outputFile);
                    out << "if ! diff"
                        << " --strip-trailing-cr -q --ignore-space-change" << arg << ";then"
                        << endl;
                    out << "diff"
                        << " --strip-trailing-cr -y --ignore-space-change" << arg << endl;
                    out << QString("echo \"Wrong answer\"") << endl;
                    out << "else" << endl;
                    out << QString("echo \"Correct answer\"") << endl;
                    out << "fi" << endl;
                ...
                if (taskList[i]->getComparisonMode() == Task::ExternalToolMode) {
                    QString arg = QString(" \"%1\" \"%2\"").arg(outputFileName, outputFile);
                    out << "if ! diff " << taskList[i]->getDiffArguments() << arg << ";then"
                        << endl;
        ...
        file.close();
#ifdef LEMON_OS_UNIX
        QProcess::execute(QString("chmod +x \"") + Settings::selfTestPath()
                          + taskList[i]->getProblemTile() + QDir::separator() + "check.sh" + "\"");
#endif
    }

    QApplication::restoreOverrideCursor();
    QMessageBox::information(widget, tr("Lemon"), tr("Self-test folder has been made"),
                             QMessageBox::Ok);
}
